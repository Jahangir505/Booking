@extends('admin.layout.master')

@section('content')
<div class="row">
    <div class="col-md-12">
        <!-- BEGIN EXAMPLE TABLE PORTLET-->
        <div class="portlet light bordered">
            <div class="portlet-title">
                <div class="caption font-dark">
                    <i class="icon-settings font-dark"></i>
                    <span class="caption-subject bold uppercase">Rate Lists</span>
                </div>
                <div class="actions">
                    <form method="POST" class="form-inline" action="{{route('search.users')}}">
                        {{csrf_field()}}
                        <input type="text" name="search" class="form-control" placeholder="Search">
                        <button class="btn btn-outline btn-circle btn-sm green" type="submit"> <i class="fa fa-search"></i></button>
                                        
                    </form>
                </div>
            </div>
            <div class="portlet-body">

                <table class="table table-striped table-bordered table-hover order-column">
                    <thead>
                        <tr>
                            <th>
                               Discount Parcentage
                            </th>
                            <th>
                                From Date
                            </th>
                            <th>
                               To Date
                            </th>
                            <th>
                                Valid For Nationality
                            </th>
                            <th>
                                Hotel Id
                            </th>
                            <th>
                                Room Id
                            </th>
                            <th>
                                edit
                            </th>
                            <th>
                                delete
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($promotions as $promotion)
                        <tr>
                            <td>
                               {{$promotion->discount}}%
                            </td>

                            <td>
                                {{$promotion->from_date}}      
                            </td> 

                            <td>
                               {{$promotion->to_date}}
                            </td>

                            <td>
                               {{$promotion->valid_for_nationality}}
                            </td>

                            <td>
                                {{$promotion->room ? $promotion->room->id : null}}   
                            </td>

                            <td>
                                {{$promotion->hotel ? $promotion->hotel->id : null}}   
                            </td>

                            <td>
                                <a href="{{route('promotions.edit', $promotion->id)}}" class="btn green">
                                    <i class="fa fa-pencil-square-o"></i> 
                                </a>
                            </td>
                            <td>
                                <form action="{{route('promotions.delete', $promotion->id)}}" method="POST">
                                    {{csrf_field()}}
                                    @method('delete')
                                    <button class="btn btn-danger" type="submit"><i class="fa fa-trash"></i> </button>
                                </form>
                            </td>
                        </tr>
                        @endforeach 
                    <tbody>
                </table>
           
        </div>
      
      </div><!-- row -->
      </div>
    </div>
  </div>    
</div>
@endsection